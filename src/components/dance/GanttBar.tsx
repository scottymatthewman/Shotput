import { getGanttBarFillStyle, ganttBarOuterRingClass } from '@/components/dance/ganttBarTaskTokens'
...
const RESIZE_PILL_BASE =
  'pointer-events-none absolute top-1/2 h-7 max-w-none -translate-y-1/2 rounded-full bg-white opacity-0 shadow-[0_1px_2px_rgba(0,0,0,0.28)] ring-1 ring-black/12 transition-opacity duration-150 ease-out peer-hover:opacity-100 peer-active:opacity-100 peer-focus-visible:opacity-100 motion-reduce:opacity-100 dark:shadow-[0_1px_2px_rgba(0,0,0,0.5)] dark:ring-white/15'
...
function ResizeEndZone({
  ...
  return (
    <div
      className="pointer-events-none absolute top-1/2 z-30 h-10 -translate-y-1/2"
      style={
        align === 'left'
    ...
    >
      <button
        type="button"
        tabIndex={-1}
        className="peer dance-focus-ring pointer-events-auto absolute top-0 bottom-0 max-w-none -translate-x-1/2 cursor-ew-resize touch-none rounded-md border-0 bg-transparent p-0 outline-none transition-surface duration-150 ease-out pressable"
...
/**
 * Gantt task bar: glass body (`rounded-md`) with white pill handles outside the ends
...
export const GanttBar = forwardRef<HTMLDivElement, GanttBarProps>(function GanttBar(
  ...
  const fillStyle = getGanttBarFillStyle(s)
  const ringClass = ganttBarOuterRingClass(s)

  return (
    <div ref={ref} className={cn('relative min-h-9 min-w-0 select-none', className)} style={style}>
      <ResizeEndZone
        align="left"
      ...
      <div className={cn('min-h-9 w-full rounded-md', ringClass)}>
          <div
            className={cn(
              'relative isolate flex min-h-9 w-full cursor-pointer items-stretch gap-1 overflow-hidden rounded-md py-1 pl-1 pr-2 active:cursor-grabbing',
            )}
          >
          <div
          ...
          {toolbar ? (
            <div className="relative z-10 flex shrink-0 items-stretch justify-center">{toolbar}</div>
          ) : null}

          <div
            className={cn(
              'relative z-[2] flex min-h-0 min-w-0 flex-1 cursor-pointer touch-none items-center gap-1 rounded-md py-px active:cursor-grabbing',
              !children ? 'min-h-6' : '',
            )}
            role="slider"
            aria-valuemin={0}
