...
import { requestTaskSheetAnimatedClose, useDanceStore } from '@/state/store'
...
function phaseDeepLink(phaseId: string, planId: string) {
  ...
  return `${base}/plan/${planId}?phase=${phaseId}`
}
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        ...
        const overviewPlanId = location.pathname.match(/^\/plan\/([^/]+)\/overview\/?$/)?.[1]
        if (
          overviewPlanId &&
          !typing &&
          !helpOpen &&
      ...
      if ((e.metaKey || e.ctrlKey) && e.shiftKey && (e.key === 'c' || e.key === 'C')) {
        if (typing) return
        e.preventDefault()
        const targetId = taskTargetId(state)
        if (targetId) {
      ...
      if (timelineRoute && !typing && !commandOpen) {
        ...
        if ((e.key === 'c' || e.key === 'C') && !e.metaKey && !e.ctrlKey && !e.altKey) {
          ...
          if (id) {
            ...
            navigate(`/plan/${timelineRoute.planId}?phase=${encodeURIComponent(id)}`)
          }
          return
        }
      }
      ...
      let phaseId: string | null = taskTargetId(state)
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription id="shortcuts-help-desc">
            ...
            and ⌘⌫. Esc closes the command menu, quick dialogs, then the task sheet.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 text-sm">
          <ShortcutSection title="General">
            ...
            <ShortcutRow keys={['Esc']} description="Close menu, dialog, or task sheet; leave overview" />
            <ShortcutRow keys={['⌘', '.']} description="Toggle sidebar / close sheet" />
