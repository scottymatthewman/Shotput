...
export function PlanBudgetTracker({ plan, workspace, planId, className }: PlanBudgetTrackerProps) {
  ...
  return (
    <div className={cn('flex min-w-0 max-w-md flex-col items-end gap-1.5', className)}>
      <div className="flex min-w-0 flex-wrap items-center justify-end gap-x-2 gap-y-0.5 text-xs">
        <span className="shrink-0 font-medium text-muted-foreground">Budget</span>
        {statusLabel ? (
          <span
            className={cn(
      ...
      {progressPct != null ? (
        <div
          className="h-1 w-32 max-w-full overflow-hidden rounded-full bg-muted"
          role="progressbar"
        ...
        >
          <div
            className={cn(
              ...
              rollup.overSpent ? 'bg-destructive' : 'bg-primary/80',
            )}
