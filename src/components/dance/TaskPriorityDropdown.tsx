...
export function TaskPriorityDropdown({
  ...
  const pendingTaskId = useDanceStore((s) => s.pendingPhasePriorityDropdownPhaseId)
  const clearPending = useDanceStore((s) => s.clearPendingPhasePriorityDropdown)
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      const next = TASK_PRIORITY_ORDER[idx]
      if (!next) return
        ...
        >
          {TASK_PRIORITY_ORDER.map((p, i) => (
            ...
            >
              ...
              <span className="min-w-0 flex-1">{taskPriorityLabel(p)}</span>
