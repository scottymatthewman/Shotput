...
export function TaskQuickActionDialogs({ planId }: { planId: string }) {
  const dialog = useDanceStore((s) => s.phaseQuickDialog)
  const setDialog = useDanceStore((s) => s.setPhaseQuickDialog)
  const updatePhaseDetails = useDanceStore((s) => s.updatePhaseDetails)
  const deletePhase = useDanceStore((s) => s.deletePhase)
  const phase = useDanceStore((s) =>
    dialog?.phaseId ? s.workspace.phases[dialog.phaseId] : undefined,
  )
  const { userIds, users } = useDanceStore(
    useShallow((s) => ({
    ...
    <>
      ...
      <Dialog open={dialog.kind === 'assigneeOwner'} onOpenChange={(v) => !v && close()}>
        ...
        >
          ...
          <div className="max-h-[min(320px,50vh)] overflow-y-auto rounded-lg border border-border/80 bg-muted/20 px-2 py-2">
