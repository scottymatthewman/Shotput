import { GanttBar } from '@/components/dance/GanttBar'
import { ganttBarForegroundClass } from '@/components/dance/ganttBarTaskTokens'
...
import { GanttCollisionBoundaryContext } from '@/components/dance/timeline/GanttCollisionBoundaryContext'
import {
  commitDayDelta,
  createGesture,
  datesAfterCommit,
  ...
  type GanttDragKind,
  type GanttGesture,
} from '@/components/dance/timeline/ganttDrag'
...
export function GanttView({
  ...
  const updatePhaseDates = useDanceStore((s) => s.updatePhaseDates)
        ...
        >
          <div className="sticky top-0 z-30 shrink-0">
            <div className="relative isolate flex min-h-10">
              ...
              <div
                className="pointer-events-none absolute inset-0 z-0 bg-[linear-gradient(to_bottom,var(--color-gantt-canvas)_0%,var(--color-gantt-canvas)_75%,transparent_100%)]"
                aria-hidden
              />
              {ticks.map((w) => {
                const outside =
              ...
              {showTodayLine ? (
                <div
                  className="pointer-events-none absolute bottom-0 z-20 size-2.5 -translate-x-1/2 translate-y-1/2 rotate-45 rounded-[1px] bg-white shadow-md ring-2 ring-gantt-canvas"
          ...
          <div className="relative flex min-h-0 min-w-0 flex-1 flex-col">
            ...
            <div className="relative isolate z-[1] min-h-0 flex-1">
              {showTodayLine ? (
                <div
                  className="pointer-events-none absolute inset-y-0 z-[2] w-px -translate-x-1/2 bg-white"
                  style={{ left: `${todayLeftPct}%` }}
                />
              ) : null}
...
const GanttTimelineRow = memo(function GanttTimelineRow({
  ...
  const start = frozen ? frozen.origStart : parseTaskCalendarDate(livePhase.start)
  const end = frozen ? frozen.origEnd : parseTaskCalendarDate(livePhase.end)
  ...
  return (
    <div
      ...
      className={cn(
        'relative flex cursor-default transition-surface duration-150 ease-hover hover:bg-muted/30',
        dragging ? 'z-20' : 'z-[3]',
...
function TrackBody({
  ...
  const nudgePhaseByDays = useDanceStore((s) => s.nudgePhaseByDays)
  ...
  return (
    <div
      ref={trackRef}
      className="relative min-h-12 w-full flex-1 touch-none bg-muted/10 py-2.5 pr-1 pl-1"
    >
      <div
            ...
            <GanttBar
              ...
              toolbar={
                ...
                >
                  <button
                    type="button"
                    className="dance-focus-ring flex h-full min-h-9 w-8 shrink-0 cursor-pointer items-center justify-center self-stretch rounded-l-[var(--radius-nested-md-p1)] rounded-r-[4px] outline-none transition-surface duration-150 ease-hover hover:bg-white/5 data-[state=open]:bg-white/5 pressable"
