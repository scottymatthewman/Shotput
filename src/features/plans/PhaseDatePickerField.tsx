...
import { clampIsoDayToRange } from '@/lib/phaseDateOrder'
...
export type PhaseDatePickerFieldProps = {
  value: string
  ariaLabel: string
  ...
  /** Inclusive ISO day bounds for scrub + calendar (start ≤ end enforcement per field). */
  minDate?: string
  maxDate?: string
 ...
 */
export function PhaseDatePickerField({
  value,
  ariaLabel,
  onChange,
  scrub = false,
  ...
  const commit = useCallback(
    (next: string) => {
      const clamped = clampIsoDayToRange(next, minDate, maxDate)
  ...
  const isDayDisabled = useCallback(
    (day: Date) => {
      ...
      return iso !== clampIsoDayToRange(iso, minDate, maxDate)
    },
    [minDate, maxDate],
  )
    ...
    <span data-phase-row-action className="inline-flex min-w-0 items-center" onClick={(e) => e.stopPropagation()}>
      ...
      >
        ...
        <PopoverContent
          align={align}
          sideOffset={6}
          className="w-auto bg-popover p-0 shadow-md"
