...
export function parsePhaseCalendarDate(iso: string): Date {
...
export function taskCalendarSpanDays(phase: Phase) {
  return Math.max(
    1,
    differenceInCalendarDays(parsePhaseCalendarDate(phase.end), parsePhaseCalendarDate(phase.start)) + 1,
  )
}
...
export function createGesture(
  ...
  return {
    ...
    pointerId,
    originX,
    origStart: parsePhaseCalendarDate(phase.start),
    origEnd: parsePhaseCalendarDate(phase.end),
